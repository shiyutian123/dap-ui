...
<template>
  <div class="dap-ui-table">
    <vxe-table
      ref="table"
      :border="
        typeof tableBaseConfig.border === 'undefined'
          ? true
    ...
    >
      ...
      <template v-for="(config, index) in tableBaseConfig.columns">
        <vxe-table-column v-if="!config.slotName" :key="index" v-bind="config">
          <template v-slot:header>
            <span :class="{ required: config.required }">{{
              config.title
            }}</span>
          </template>
  ...
  computed: {
    computTableData: function() {
      if (this.tableBaseConfig.frontPaging) {
        const start =
          (this.tableBaseConfig.tablePage.currentPage - 1) *
          this.tableBaseConfig.tablePage.pageSize;
